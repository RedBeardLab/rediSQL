...
extern "C" fn reply_create_statement(ctx: *mut r::ffi::RedisModuleCtx,
                         ...
                         -> i32 {
    let result =
        unsafe { r::ffi::RedisModule_GetBlockedClientPrivateData.unwrap()(ctx) as *mut Result<r::StatementPresent, sql::SQLite3Error>};
    let result: Box<Result<r::StatementPresent, sql::SQLite3Error>> =
        unsafe { Box::from_raw(result) };
    match *result {
        Ok(r::StatementPresent::No) => r::reply_with_ok(ctx),
        Ok(r::StatementPresent::Yes { stmt }) => {
            let s = format!("Statement saved. Alert! The identifier \
                             was bound to another statement: \"{}\"",
                            stmt);
            r::reply_with_simple_string(ctx, s)
        }
        Err(error) => error.reply(ctx),
    }
}
...
#[allow(non_snake_case)]
extern "C" fn CreateStatement(ctx: *mut r::ffi::RedisModuleCtx,
                              argv: *mut *mut r::ffi::RedisModuleString,
                              argc: ::std::os::raw::c_int)
                              -> i32 {
                    ...
                    match sql::open_connection(path.clone()) {
                        ...
                        Err(_) => {
                            let error = CString::new("Err - Error \
                                                      ...
                                                      memory databade")
                                .unwrap();
                            unsafe { r::ffi::RedisModule_ReplyWithError.unwrap()(ctx, error.as_ptr()) }
                        }
                    }
                }
                _ => {
...
fn register_function(ctx: *mut r::ffi::RedisModuleCtx,
                    name: String,
    ...
    if unsafe {
        r::ffi::RedisModule_CreateCommand.unwrap()(ctx,
                                                   command_ptr_name,
                                                   create_db,
                                                   flag_ptr_name,
                                                   0,
...
#[allow(non_snake_case)]
#[no_mangle]
pub extern "C" fn RedisModule_OnLoad(ctx: *mut r::ffi::RedisModuleCtx,
                                     _argv: *mut *mut r::ffi::RedisModuleString,
                                     _argc: i32)
                                     -> i32 {
    ...
    if unsafe {
        r::ffi::Export_RedisModule_Init(ctx,
                                        module_ptr_name,
                                        1,
    ...
    unsafe {
        r::ffi::DBType =
            r::ffi::RedisModule_CreateDataType.unwrap()(ctx,
                                                   ptr_data_type_name,
                                                   1,
                                                   &mut types);
    }
